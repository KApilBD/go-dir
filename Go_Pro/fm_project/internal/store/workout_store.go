...
type PosgresWorkoutStore struct {
	db *sql.DB
}

func NewPosgresWorkoutStore(db *sql.DB) *PosgresWorkoutStore {
	return &PosgresWorkoutStore{db: db}
}
...
func (pg *PosgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	tx, err := pg.db.Begin()
	if err != nil {
		return nil, err
	}
...
func (pg *PosgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	tx, err := pg.db.Begin()
	if err != nil {
		return err
	}
...
func (pg *PosgresWorkoutStore) DeleteWorkout(id int64) error {
	query := `
